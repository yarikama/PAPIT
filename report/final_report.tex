...
\begin{pgfonlayer}{background}
  \node[draw=blue!30, fill=blue!4, rounded corners=4pt,
        ...
        label={[font=\scriptsize\bfseries, blue!60]above:Stage 1 — Feature Extraction}] {};
  \node[draw=red!30, fill=red!4, rounded corners=4pt,
        fit=(score)(topk),
        label={[font=\scriptsize\bfseries, red!60]above:Stage 2 — Pruning}] {};
  \node[draw=green!50, fill=green!6, rounded corners=4pt,
        fit=(mlp)(llava)(out),
        label={[font=\scriptsize\bfseries, green!50!black]above:Stage 3 — LLM}] {};
\end{pgfonlayer}
\end{tikzpicture}}
...
% TODO (CLIP insight): Explain why PAPIT's scoring signal does not
% consistently outperform random pruning.
% Suggested points to cover:
%   - CLIP's vision encoder and LLaVA's ViT are not jointly trained with
%     the LLM, so the saliency signal does not directly predict what
%     LLaVA attends to during generation.
%   - GradCAM captures text-image alignment at the CLS level; individual
%     patch gradients are diffuse when the query is satisfied by global
%     context rather than a localized region.
%   - LLaVA's strong language prior allows it to recover from suboptimal
%     patch selection, reducing the marginal value of prompt-aware scoring.
%   - Future directions: jointly trained pruning predictors or using
%     LLM-internal attention signals (FastV-style) for more reliable scoring.
