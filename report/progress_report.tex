...
%
% Uses neurips_2024.sty (already in proposal/).
% Copy neurips_2024.sty into report/ before compiling:
%   cp proposal/neurips_2024.sty report/
%
% Figures needed before submission:
%   fig_pipeline.pdf   — TikZ diagram (compile from proposal.tex or copy)
...
% ── TikZ pipeline (same as proposal) ─────────────────────
\begin{figure}[t]
\centering
...
%% ── INSTRUCTIONS ──────────────────────────────────────────
%% Run demo.ipynb Cell 10 (heatmap) and Cell 8 (patch rects),
%% then save with:
%%   fig.savefig("../report/fig_qualitative.pdf", bbox_inches="tight")
%% Place both in one side-by-side figure in the PDF.
%% Remove this comment block when the figure is ready.
%% ──────────────────────────────────────────────────────────

\begin{figure}[t]
  \centering
  % Replace with: \includegraphics[width=\columnwidth]{fig_qualitative.pdf}
...
\midrule
100\% & 49 & 1.000 & 1.000 & 62  & 4{,}577 \\
75\%  & 37 & 0.755 & 0.772 & 27  &   132 \\
50\%  & 24 & 0.490 & 0.558 & 29  &   311 \\
25\%  & 12 & 0.245 & 0.392 & 28  &   321 \\
\bottomrule
\end{tabular}
\end{table}

Key observations:
(1) Pruning latency is constant at $\approx$28\,ms regardless of $k$,
dominated by the CLIP text forward pass.
(2) E2E latency decreases substantially with more aggressive pruning,
demonstrating practical wall-clock savings.
(3) The \texttt{k=100\%} row has higher E2E because it uses the full
unpruned BLIP pipeline without the token-selection shortcut.
...
\centering
\small
\caption{Planned main accuracy results.
         ``??'' marks pending experiments.
...
(2) Generate the accuracy–efficiency Pareto curve (Figure~\ref{fig:qualitative}
is a placeholder; the Pareto curve will replace it);
